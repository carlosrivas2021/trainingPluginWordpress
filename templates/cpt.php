...
<?php
$options = get_option('crivas_cpt') ?: array();
...
foreach ($options as $option) {
    ...
    settings_fields('crivas_plugins_cpt_settings');
    echo '<input type="hidden" name="remove" value="' . $option['post_type'] . '">';
    submit_button('Delete', 'delete small', 'submit', false, array(
        'onclick' => 'return confirm("Are you sure you want to delete this Custom Post Type? The data associate with it will not be deleted.");',
    ));
    echo ' </form>';
    echo '</td></tr>';
}
...
?>
    ...
    <div id="tab-2" class="tab-pane <?php echo (isset($_POST["edit_post"])) ? 'active' : '' ?>">
      ...
      <?php
settings_fields('crivas_plugins_cpt_settings');
do_settings_sections('crivas_cpt');
submit_button();
?>
      </form>
    </div>
    ...
    <div id="tab-3" class="tab-pane">
      <h1>Export your custom post type</h1>
